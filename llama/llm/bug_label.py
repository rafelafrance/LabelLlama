...
class BugLabel(Signature):
    ...
    identifiedByID: list[str] = OutputField(desc=identified_by_id.IDENTIFIED_BY_ID)
    occurrence_id: list[str] = OutputField(desc=occurrence_id.OCCURRENCE_ID)
    ...
    water_body: list[str] = OutputField(desc=water_body.WATER_BODY)
    island: list[str] = OutputField(desc=island.ISLAND)
    island_group: list[str] = OutputField(desc=island_group.ISLAND_GROUP)
