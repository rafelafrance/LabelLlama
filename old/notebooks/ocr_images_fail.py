...
def _():
    ...
    return OcrImage, Path, base64, dataclass, datetime, dspy, duckdb
...
@app.cell
def _(Path, duckdb):
    db_path = Path("data/herbarium/labelllama_herbarium_test.db")
    cxn = duckdb.connect(db_path)
    return cxn, db_path
...
def _(Path, dataclass, db_path):
    ...
    class Args:
        ...
        # Model parameters
        model_name: str = "lm_studio/Chandra-OCR-GGUF"
        api_host: str = "http://localhost:11434"  # URL for the LM model
        api_key: str = ""
...
@app.cell
def _(Args, OcrImage, Path, base64, datetime, dspy, duckdb, filter_images, mo):
    def ocr_images(args: Args) -> None:
        ...
        predictor = dspy.Predict(OcrImage)
        ...
        with duckdb.connect(args.db) as cxn:
            ...
            for image_path in mo.status.progress_bar(image_paths, title="OCR Progress"):
                ...
                with Path(image_path).open("rb") as f:
                    ...
                    prediction = predictor(image=encoded)
...
@app.cell
def _(Args, Path, ocr_images):
    args1 = Args(
        model_name="ollama_chat/gemma3:12b",
